...
Var VCREDIST_OK
Section "-SEC_VCREDIST_X86"
    MessageBox MB_OK|MB_ICONINFORMATION \
        "即将执行 “适用于 Visual Studio 2015、2017 和 2019 的 Microsoft Visual C++” 的 32-bits 检测程序。期间可能会出现错误提示，但这并不表示安装失败。$\r$\n$\r$\n按 “确定” 开始检测。"
    StrCpy $VCREDIST_OK ""
    SetOutPath "$PLUGINSDIR\vcredist"
    File "..\x86\Release\DummyCppConsoleApp.exe"
    ...
    ${If} $VCREDIST_OK == ""
        ...
        true:
            SetCompress off
            File "deps\VC_redist.x86.exe"
            SetCompress auto

            DetailPrint "Visual C++ Redistributable 正在安装 ..."
            ExecWait '"$OUTDIR\VC_redist.x86.exe" /norestart /passive' $0
            BringToFront
            ${If} $0 != "0"
