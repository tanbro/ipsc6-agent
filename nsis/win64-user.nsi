...
!define PRODUCT_NAME "IPSC6 座席工具条 (x64 User)"

Name "${PRODUCT_NAME}"
!ifdef VERSION
  OutFile "out\ipsc6_agent_wpfapp-win64-user.${VERSION}.exe"
!else
...
InstallDirRegKey HKCU "Software\ipsc6_agent_wpfapp-win64" ""
...
Section "!座席工具条" SEC_0
  ...
  WriteRegStr HKCU "Software\ipsc6_agent_wpfapp-win64" "" $INSTDIR
  ...
  ; Add/Remove list
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\ipsc6_agent_wpfapp-win64" \
...
Section "!un.座席工具条" UNSEC_0
  ...
  DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\ipsc6_agent_wpfapp-win64"

  ;Remove InstalledRegKey only when it is empty
  DeleteRegKey /ifempty HKCU "Software\ipsc6_agent_wpfapp-win64"
SectionEnd
