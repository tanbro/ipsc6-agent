...
座席程序**不提供**安全的 WebSocket 连接(无 `wss://`, 仅 `ws://`) ，其默认的 URL 是 `ws://localhost:9876/ws` 或者 `ws://127.0.0.1:9876/ws` ， 对应的 origin 分别是 `http://localhost:9876` 和 `http://localhost:9876` 。

从非安全 (**非** `HTTPS`) 的 Origin 的页面访问上述地址并不会被浏览器安全策略拦截。
...
- Safari 不允许任何混合内容。(座席程序仅支持 Windows，此情况不予考虑)
...
[^1]: <https://developer.mozilla.org/en-US/docs/Web/Security/Mixed_content#loading_mixed-content_resources>
