...
外网地址
: 应用程序可以指定此字段的任何地址或主机名，例如它可以指向系统中的网口地址之一，也可以指向已为应用程序配置端口映射的 NAT 路由器的公共地址。
...
本地 SIP 网络端口
: 座席程序的软电话模块在所有的绑定的网络接口上使用这个端口进行双向的 SIP 网络通信。

    | key                  | type      | required | default |
    | -------------------- | --------- | -------- | ------- |
    | `Phone:LocalSipPort` | `Integer` |          | `5060`  |
